...
class ETLSwiftieChatPipeline:
    def __init__(
        ...
        self.collection_db = ExtractCollectionDB(
            collection_name, file_summary_songs, data_path, vectordb_file_name
        )
        self.genrative_model_tools = GenerativeModelTools(self.collection_db)
        self.swiftie_chat = SwiftieChat(
            self.genrative_model_tools, swiftie_chat_instructions
        )
...
if __name__ == "__main__":
    ...
    etl_assistant_pipeline = ETLSwiftieChatPipeline(
        collection_name, file_summary_songs, swiftie_chat_instructions
    )
