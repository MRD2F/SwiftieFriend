...
class GenerativeModelTools:
    ...
    def get_complete_lyrics(
        self, song_name: str, album_name: str = "", threshold: int = 87
    ) -> dict:
        ...
        for song, album in song_album.items():
            ...
            results = self.vectordb._collection.get(
                where={"$and": [{"song": {"$eq": song}}, {"album": {"$eq": album}}]}
            )
    ...
    def get_album_songs(self, album_name: str, threshold: int = 90):
        ...
        for stored_album in self.album_songs_summary.keys():
            ...
            score = fuzz.partial_ratio(
                stored_album_spaced, space_song_names(album_name)
            )
            if score >= threshold:
        ...
        for album in matches_albums:
            ...
            if (n == 1) & (
                space_song_names(album_name).lower() in split_by_capitals(album)
            ):
                album_tracks[space_song_names(album)] = [
                    space_song_names(i) for i in self.album_songs_summary[album]
                ]
            elif n > 1:
                album_tracks[space_song_names(album)] = [
                    space_song_names(i) for i in self.album_songs_summary[album]
    ...
    def get_song_match(self, query: str, n_results: int = 10):
        ...
        retriever = self.vectordb.as_retriever(
            search_type="mmr", search_kwargs={"k": n_results}
        )
    ...
    def classify_mood(self, query: str) -> str:
        ...
        chat = self.client.chats.create(
            model=self.sentiment_generative_model,
